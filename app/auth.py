# Update the import statement for BaseSettings
...
def Genearte_OTP(payload:schemas.UserRequestSchema):
    otp_base32 = pyotp.random_base32()
    otp_auth_url = pyotp.totp.TOTP(otp_base32).provisioning_uri(name="admin@admin.com",issuer_name="admin")

    if not ObjectId.is_valid(payload.user_id):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,detail=f"Invalid id: {payload.user_id}")
    updated_user = User.find_one_and_update({'_id':ObjectId(payload.user_id)},{'$set':{'otp_auth_url':otp_auth_url,"otp_base32":otp_base32}},return_document=ReturnDocument.AFTER)
...
def Verify_OTP(payload:schemas.UserRequestSchema):
    ...
    user = userEntity(User.find_one({'_id':ObjectId(payload.user_id)}))
    if not user:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,detail=message)

    print(user.get('otp_base32'))
    totp = pyotp.TOTP(user.get('otp_base32'))
    print('totp: {0}'.format(totp))
    if not totp.verify(payload.token):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,detail=message)
    
    updated_user = User.find_one_and_update({'_id':ObjectId(payload.user_id)},{'$set': {"otp_enabled": True, "otp_verified": True}}, return_document=ReturnDocument.AFTER)
...
def Disable_OTP(payload:schemas.UserRequestSchema):
    ...
    updated_user = User.find_one_and_update({'_id': ObjectId(payload.user_id)}, {'$set': {"otp_enabled": False}}, return_document=ReturnDocument.AFTER)
